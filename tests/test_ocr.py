"""Unit tests for OCR: PaddleOCR result normalization + region->chunk mapping (engine mocked)."""
...
def test_line_boxes_two_tuple_paragraph():
    # paragraph=True returns 2-tuples
    res = [
        (
            [[0, 0], [50, 0], [50, 20], [0, 20]],
            "para text",
...
def test_transcribe_maps_lines_to_regions(monkeypatch):
    # one page with two regions; lines fall inside region 1 only
    page_id = "bookA_p0001"
    regions = [
    ...
    cfg = {"ocr": {"model": "paddleocr:PP-OCRv5_mobile", "lang": "en"}}
